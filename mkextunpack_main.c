...
*******************************************************************************/
void usage(int num) {
    fprintf(stderr, "usage: %s [-v] [-a arch] [-c compression] [-d output_dir] mkextfile\n", progname);
    fprintf(stderr, "    -d output_dir: where to put kexts (must exist)\n");
    fprintf(stderr, "    -a arch: pick architecture from fat mkext file\n");
    fprintf(stderr, "    -c compression: pick compression of the fat mkext file (valid: lzss/lzvn)\n");
...
int main (int argc, const char * argv[]) {
    ...
    uint32_t           mkextVersion;
    unsigned char      mkextCompression = 0;
    ...
    while ((optchar = getopt(argc, (char * const *)argv, "a:d:hv")) != -1) {
        switch (optchar) {
          ...
          case 'c':
            ...
            if (!strncasecmp(optarg, "lzss", 4))
            {
                mkextCompression = 0;

            } else if (!strncasecmp(optarg, "lzvn", 4)) {
                mkextCompression = 1;
            } else {
                fprintf(stderr, "Compression '%s' not valid\n", optarg);
                usage(0);
                exit_code = 1;
                goto finish;
            }
    ...
    */
    ...
    if (mkextVersion == MKEXT_VERS_2) {
        ...
        entries = extractEntriesFromMkext1(mkextStart, mkextEnd, mkextCompression);
        if (!entries) {
    ...
    for (i = 0; i < MKEXT_GET_COUNT(mkextHeader); i++) {
        ...
        */
        if (!uncompressMkext1Entry(mkextStart,
            plist_file, &kextPlistDataObject, compression) || !kextPlistDataObject) {
        ...
        */
        if (OSSwapBigToHostInt32(module_file->offset) ||
            ...
            if (!uncompressMkext1Entry(mkextStart,
                module_file, &kextExecutable, compression)) {
    ...
    unsigned char compressionType)
{
    ...
    */
    ...
    if (compsize != 0) {
        ...
        {
            uncompressed_size = lzvn_decode(uncompressed_data,
                                            (u_int32_t)realsize,
